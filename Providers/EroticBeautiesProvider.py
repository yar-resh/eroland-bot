...
class EroticBeautiesProvider(EroBaseProvider):
    ...
    def get_random_images(self, amount):
        ...
        for number in random_page_numbers:
            page = self._page_url_template.format(number)
            request_url = self.request_url + page
            response = self._session.get(url=request_url)
            bs = BeautifulSoup(response.text, 'lxml')
            content = bs.find('div', class_='col-xs-12 col-sm-8 col-md-5 col-lg-6 pull-left text-center center-block index-content')
            ...
            galery = bs.find('div', class_='my-gallery')
            random_image = choice(galery.find_all('figure'))
